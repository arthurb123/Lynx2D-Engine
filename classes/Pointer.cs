    ...
    class Pointer
    {
        ...
        public static void Inject(string target)
        {
            ...
            Engine.ExecuteScript("var enginePointer = new lx.GameObject(new lx.Sprite('res/lynx2d/pointer.png'), 0, 0, 12, 12).Show(lx.GAME.BUFFER.length+1);" +
                                 ...
                                 "var enginePointerLoopID = lx.Loops(function() { if (" + target + " == undefined) return; var tX = " + target + ".Position().X; var tY = " + target + ".Position().Y; enginePointer.Position(tX, tY); enginePointerText.Text(Math.round(tX) + ' , ' + Math.round(tY)); }).GAME.LOOPS.length-1;");
